module vector_rot3d_m
!=======================================================
! Calculation of the coordinates of the rotated vector
!=======================================================
 implicit none
 private
 save
 ...
 type vector_rot3d
  ...
  logical :: rm_not_ready=.true.
 contains
  procedure :: set=>set_nrot, del=>del_nrot, set_rot, calc_rm
  procedure :: rotate, rotate_back
 end type vector_rot3d
...
! we use transposed matrices, so we multiply in opposite order
 ...
 this%rm_not_ready=.false.
end subroutine calc_rm

subroutine rotate(this,x,xnew)
 class(vector_rot3d) :: this
 real(8), intent(in) :: x(3)
 real(8), intent(out) :: xnew(3)
! rotation of vector
 if (this%rm_not_ready) call this%calc_rm
...
subroutine rotate_back(this,xnew,x)
 class(vector_rot3d) :: this
 real(8), intent(in) :: xnew(3)
 real(8), intent(out) :: x(3)
! rotation of vector back
 if (this%rm_not_ready) call this%calc_rm
...
end module vector_rot3d_m


!program main
! use vector_rot3d_m, only : vector_rot3d
! type(vector_rot3d) :: rt3d
! real(8) :: x(3), xnew(3)
!  
! x=[0d0,0d0,1d0]

! call rt3d%set(3)
! call rt3d%set_rot(1,2,45d0)
! call rt3d%set_rot(2,3,45d0)
! call rt3d%set_rot(3,1,90d0)
!! call rt3d%set_rot(1,1,-50d0)


! call rt3d%rotate(x,xnew)
! write(*,'(A,3Es14.6)') 'xnew=', xnew
! call rt3d%rotate_back(xnew,x)
