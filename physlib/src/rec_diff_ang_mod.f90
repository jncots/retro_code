...
! Example:
! ========
! use rec_diff_ang_mod, only : rec_diff_ang
! type(rec_diff_ang) :: this
! call this%init(diff,qs)
! call this%cr_den(en,t,r,nu,res) ! [t]=sec, [r]=cm, res=[1/(cm^3 eV)]
! nu=1-mu, where mu=cos(theta), where theta is pitch angle relative
! radial direction
! =========
! Here:
...
!================================================================
 ...
 type rec_diff_ang
  real(8), private :: en, xi, trc, tobs, nu
 ...
 end type rec_diff_ang
 
 
 type(rec_diff_ang), pointer :: loc
...
subroutine cr_den(this,en,tobs,robs,nu,res)
 class(rec_diff_ang), target :: this
 real(8), intent(in) :: en, tobs, robs, nu
 real(8), intent(out) :: res
...
subroutine icr_den(x,res)
 ...
 call pjun0(x,loc%xi,loc%nu,pres)
...
subroutine pjun0(s,xi,nu,res)
 real(8), intent(in) :: s, xi, nu
 real(8), intent(out) :: res
